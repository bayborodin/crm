...
def index(request):
    ...
    shipment_statistics_previous = WeekResult.objects.filter(
        metric=shipment_metric, year=week_ago.year, week=week_ago.isocalendar()[
            1],
    )
    ...
    payment_statistics_previous = WeekResult.objects.filter(
        metric=payment_metric, year=week_ago.year, week=week_ago.isocalendar()[
            1],
    )
    ...
    if calls_cnt_prev_week == 0:
        ...
        calls_cnt_percent = get_metrics_rate(
            calls_cnt_prev_week, calls_cnt_cur_week,
        )
