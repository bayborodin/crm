...
class LeadSource(models.Model):
    name = models.CharField(max_length=250, db_index=True, verbose_name='Наименование')
    secret_key = models.CharField(max_length=250, db_index=True, verbose_name='Секретный ключ')
    description = models.CharField(max_length=250, verbose_name='Описание', blank=True)
...
class Lead(models.Model):
    ...
    sale_channels = models.CharField(max_length=250, verbose_name='Каналы продаж', blank=True)
    sale_regions = models.CharField(max_length=250, verbose_name='Регионы присутствия', blank=True)
    sale_points = models.CharField(max_length=250, verbose_name='Точки продаж', blank=True)
    web_address = models.CharField(max_length=250, verbose_name='Адрес сайта', blank=True)
    ...
    first_person_position = models.CharField(max_length=250, verbose_name='Должность', blank=True)
    ...
    bank_corr_account = models.CharField(max_length=250, verbose_name='Корр. счет', blank=True)
    contact_person = models.CharField(max_length=250, verbose_name='Контактное лицо', blank=True)
    contact_phone = models.CharField(max_length=250, verbose_name='Телефон для связи', blank=True)
    contact_email = models.CharField(max_length=250, verbose_name='Email для связи', blank=True)
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
