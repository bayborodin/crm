    ...
    def __init__(self, account, *args, **kwargs):
        ...
        if 'shipment' in self.data:
            try:
                ...
                shipment_offerings = ShipmentOffering.objects.filter(shipment_id=shipment_id).all()
                ids = []
                for shipment_offering in shipment_offerings:
                    ids.append(shipment_offering.offering.id)
                self.fields['offering'].queryset = Offering.objects.filter(id__in=ids).order_by('name')

            except(ValueError, TypeError) as e:
                print(e)
        elif self.instance.pk:
            shipment_offerings = ShipmentOffering.objects.filter(shipment=self.instance.shipment).all()
            ids = []
            for shipment_offering in shipment_offerings:
                ids.append(shipment_offering.offering.id)
            self.fields['offering'].queryset = Offering.objects.filter(id__in=ids).order_by('name')
